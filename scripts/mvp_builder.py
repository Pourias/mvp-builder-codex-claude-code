...
MODEL_CONTEXT_ENV_VARS = ("OPENAI_MODEL", "CODEX_MODEL")
REASONING_CONTEXT_ENV_VARS = ("OPENAI_REASONING_EFFORT", "CODEX_REASONING_EFFORT")
QUOTE_PAIRS = {
    '"': '"',
    "'": "'",
    "“": "”",
...
STATE_PROMPTS = {
    "BOOTSTRAP_CODEX": "bootstrap_verify_environment",
...
def derive_codex_session_status(thread_id: str) -> str:
    return "bound_to_conversation" if thread_id else "conversation_not_exposed"


def sync_codex_session(run_dir: Path, run_spec: dict) -> tuple[dict, bool]:
    codex_session_path = run_dir / "codex_session.json"
    codex_session = load_json(codex_session_path)
    changed = False

    thread_id = first_env_value(THREAD_CONTEXT_ENV_VARS)
    desired = {
        "provider": "codex-skill",
        "transport_mode": str(run_spec.get("transport_mode", "codex-skill")).strip() or "codex-skill",
        "binding_scope": "codex_conversation_if_available",
        ...
        "status": derive_codex_session_status(thread_id),
        "workspace_path": run_spec["workspace_path"],
        "source_channel": "",
        "source_peer_or_thread": "",
        "created_at": str(codex_session.get("created_at", "")).strip() or now_iso(),
        "last_used_at": now_iso(),
        "updated_at": now_iso(),
    }

    for key, value in desired.items():
        if codex_session.get(key) != value:
            codex_session[key] = value
            changed = True

    if changed:
        write_json(codex_session_path, codex_session)
    return codex_session, changed
...
def render_status(run_spec: dict, state: dict, codex_session: dict) -> str:
    ...
    lines = [
        ...
        *(
        ...
        f"- codex_session_id: {codex_session.get('session_id', '')}",
        f"- session_key_origin: {codex_session.get('session_key_origin', '')}",
        f"- codex_thread_id: {codex_session.get('thread_id', '') or 'unmapped'}",
        f"- codex_session_status: {codex_session.get('status', '')}",
        f"- codex_model: {codex_session.get('model', '')}",
        f"- codex_workspace_path: {codex_session.get('workspace_path', '')}",
    ]
...
def render_latest_update(
    ...
    lines.extend(
        [
            "",
            "Human update:",
            f"{happened}. Decision: `{decision}`. Next state: `{next_state}`. Run status: `{state['status']}`. Codex thread: `{codex_session.get('thread_id', '') or 'unmapped'}`.",
        ]
    )
    return "\n".join(lines).strip() + "\n"
...
def human_state_headline(state: dict) -> str:
    ...
    if current_state == "BOOTSTRAP_CODEX":
        return "Verifying the Codex environment and runtime setup."
    if current_state == "RESEARCH_ARCHITECTURE":
...
def human_state_detail(run_spec: dict, state: dict, codex_session: dict) -> str:
    ...
    thread_id = str(codex_session.get("thread_id", "") or "unmapped").strip()
    ...
    details.append(f"Codex thread: `{thread_id}`.")
...
def human_progress_payload(run_spec: dict, state: dict, codex_session: dict) -> dict:
    ...
    return {
        ...
        "codex_thread_id": str(codex_session.get("thread_id", "") or "unmapped").strip(),
...
def sync_research_state(run_dir: Path, run_spec: dict, state: dict, codex_session: dict) -> tuple[dict, dict, bool]:
    with run_lock(run_dir, "research-sync.lock"):
        ...
        codex_session = load_json(run_dir / "codex_session.json")
        ...
        def activate_fallback(reason: str, error_message: str = "") -> tuple[dict, dict, bool]:
            ...
            if (
                ...
                state["last_decision"] = "fallback_to_codex_search"
                ...
                latest = render_latest_update(
                    ...
                    state=state,
                    codex_session=codex_session,
                    happened=f"Brokered research fell back to Codex search for {target_label}",
                    decision=state["last_decision"],
                    next_state=state["current_state"],
                    note=(
                        f"{reason.strip()} "
                        f"Use the fallback prompt to have Codex perform regular online search directly for {target_label}."
                    ),
                )
                write_json(run_dir / "state.json", state)
                write_views(run_dir, run_spec, state, codex_session, latest)
        ...
        if (
            ...
            sticky_reason = (
                ...
                "Skipping another brokered wait and falling back to Codex regular search."
            )
        ...
        if response_status in {"failed", "stale"}:
            ...
            fallback_reason = (
                ...
                f"Falling back to Codex regular search."
            )
...
def fail_run(run_dir: Path, run_spec: dict, state: dict, reason: str) -> None:
    codex_session, _ = sync_codex_session(run_dir, run_spec)
    state["status"] = "failed"
...
def init_run(args: argparse.Namespace) -> int:
    ...
    effective_transport_mode = "codex-skill"
    openclaw_session_key = ""
    binding_scope = "codex_conversation_if_available"
    session_key_origin = ""
    transport_note = (
        "This run is self-contained inside the MVP Builder Codex skill. "
        "Codex may record the current thread id when the environment exposes it, but the run does not depend on OpenClaw services."
    )
    ...
    write_json(run_dir / "codex_session.json", codex_session)
    codex_session, _ = sync_codex_session(run_dir, run_spec)
    ...
    init_note = "Render the current prompt, complete that state inside Codex, then apply the structured reply back to the run."
    if transport_note:
    ...
    if codex_session.get("thread_id"):
        print(f"CODEX_THREAD_ID={codex_session['thread_id']}")
    return 0
...
def apply_reply_run(args: argparse.Namespace) -> int:
    ...
    if (
        ...
        and state.get("current_state") == "BOOTSTRAP_CODEX"
    ...
    if reply_class == "COMPLETED":
        ...
        if current_state == "ENUMERATE_STAGES":
            ...
            final_report.extend(
                [
                    ...
                    "## Codex Context",
                    "",
                    f"- Codex thread id: {codex_session.get('thread_id', '') or 'unmapped'}",
        ...
        latest = render_latest_update(
            ...
            state=state,
            codex_session=codex_session,
            happened=f"Codex completed state {current_state}",
            decision="advance_state",
            next_state=state["current_state"],
    ...
    if reply_class == "RECOMMENDED_OPTION":
        ...
        if not choice:
            ...
            latest = render_latest_update(
                ...
                state=state,
                codex_session=codex_session,
                happened=f"Codex returned a recommended option in {current_state}",
    ...
    if reply_class in {"NEEDS_SUGGESTION", "NEEDS_IMPLEMENT_CONFIRMATION"}:
        ...
        latest = render_latest_update(
            ...
            state=state,
            codex_session=codex_session,
            happened=f"Codex requested follow-up in {current_state}",
            decision=state["last_decision"],
            next_state=current_state,
    ...
    if reply_class == "STATUS_ONLY":
        ...
        latest = render_latest_update(
            ...
            state=state,
            codex_session=codex_session,
            happened=f"Codex reported progress in {current_state}",
            decision="hold_state",
            next_state=current_state,
    ...
    if reply_class == "BLOCKED_NEEDS_HUMAN":
        ...
        latest = render_latest_update(
            ...
            state=state,
            codex_session=codex_session,
            happened=f"Codex is blocked in {current_state}",
            decision="pause_for_human",
            next_state=current_state,
            note=reply_summary,
        )
...
def apply_human_feedback_run(args: argparse.Namespace) -> int:
    ...
    if decision == "change":
        ...
        latest = render_latest_update(
            ...
            decision=state["last_decision"],
            next_state=state["current_state"],
            note="Render `request_contract_revise` and send it through the Codex transport.",
        )
        write_json(run_dir / "state.json", state)
        write_views(run_dir, run_spec, state, codex_session, latest)
        append_jsonl(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Self-contained state-machine runner for the MVP Builder Codex skill.")
    ...
    reply_parser = subparsers.add_parser("apply-reply", help="Apply a Codex reply to the current run.")
